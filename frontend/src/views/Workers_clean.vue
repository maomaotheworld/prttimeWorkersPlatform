  ...
  <div class="workers-container">
    ...
    >
      <div class="import-content">
        ...
        >
          <div>
            必要欄位：編號(A欄)、姓名(B欄)、組別(C欄)、樓層(D欄)、時薪(E欄)
          </div>
        </el-alert>
        ...
        <div v-if="previewData.length > 0" class="preview-section">
          ...
          <div class="preview-stats">
            <span
              >總計 {{ previewData.length }} 筆，有效 {{ validCount }} 筆，錯誤
              {{ previewData.length - validCount }} 筆</span
            >
          </div>
        </div>
      </div>
    ...
    >
      <div class="wage-adjust-content">
        ...
        <el-form
          ref="wageFormRef"
          :model="wageAdjustForm"
          :rules="wageAdjustRules"
          label-width="100px"
        >
      ...
      <template #footer>
        ...
        <el-button
          type="primary"
          @click="submitWageAdjust"
          :loading="adjustingWage"
        >
          確認調整
        </el-button>
      </template>
    ...
    >
      <div class="hours-adjust-content">
        ...
        <el-form
          ref="hoursFormRef"
          :model="hoursAdjustForm"
          :rules="hoursAdjustRules"
          label-width="100px"
        >
      ...
      <template #footer>
        ...
        <el-button
          type="primary"
          @click="submitHoursAdjust"
          :loading="adjustingHours"
        >
          確認調整
        </el-button>
      </template>
...
const formRules = {
  ...
  floor: [{ required: true, message: "請選擇樓層", trigger: "change" }],
  hourlyWage: [{ required: true, message: "請輸入基本時薪", trigger: "blur" }],
};

const wageAdjustRules = {
  newWage: [{ required: true, message: "請輸入新時薪", trigger: "blur" }],
  reason: [{ required: true, message: "請輸入調整原因", trigger: "blur" }],
};

const hoursAdjustRules = {
  hours: [{ required: true, message: "請輸入調整時數", trigger: "blur" }],
  date: [{ required: true, message: "請選擇調整日期", trigger: "change" }],
  reason: [{ required: true, message: "請輸入調整原因", trigger: "blur" }],
};
